...
//        if (list->next->location) printf(",\n");
    }
}

void json_format_list_item_endl(L_NODE *list, int endl) {
...
void json_format_list(L_NODE *list, char **locations){
    ...
    if (*locations) {
        while (*locations++)
            json_format_list_item_endl(seach_list_by_location(list, locations[-1]), !!*locations);
    } else {
        foreach(list, &json_format_list_item, TRAVERSE_FORWARD);
    }
    printf("\n]\n");
}
