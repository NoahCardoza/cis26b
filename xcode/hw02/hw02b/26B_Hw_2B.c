...
S_NODE 	*init_snode(void);
S_NODE 	*push(S_NODE *stack, int data);
void 	free_snodes(S_NODE **node);
L_NODE 	*init_lnode(void);
L_NODE 	*init_list(void);
...
S_NODE *init_snode(){
    ...
    node = (S_NODE *) my_alloc(sizeof(S_NODE), "init_snode");
    node->data = 0;
    node->next = NULL;
...
L_NODE *init_lnode(){
    ...
    node = (L_NODE *) my_alloc(sizeof(L_NODE), "init_lnode");
    node->location = NULL;
